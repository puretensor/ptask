...
mod tests {
    ...
    fn pt1687_repeat_capture_returns_the_same_row_and_flags_duplicate() {
        let (_dir, db) = fresh_db();
        let (first, dup1) =
            insert_idempotent(&db, "deploy the thing", "mcp", "mcp://hal/k1").unwrap();
        let (second, dup2) =
            insert_idempotent(&db, "deploy the thing", "mcp", "mcp://hal/k1").unwrap();
    ...
    fn pt1687_same_text_from_a_different_source_is_not_a_duplicate() {
        ...
        assert!(
            !dup,
            "identity is (source_file, text) — a different source is a different fact"
        );
        assert_ne!(a.id, b.id);
        assert_eq!(unprocessed_count(&db).unwrap(), 2);
    }
    ...
    fn pt1687_concurrent_captures_of_the_same_fact_produce_exactly_one_row() {
        ...
        for _ in 0..8 {
            ...
            handles.push(std::thread::spawn(move || {
                insert_idempotent(&db, "racy capture", "mcp", "mcp://hal/race")
                    .map(|(r, d)| (r.id, d))
            }));
        }
        ...
        assert!(
            !ok.is_empty(),
            "at least one concurrent capture must succeed"
        );
