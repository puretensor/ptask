...
complete -c pt -n "__fish_pt_using_subcommand help; and not __fish_seen_subcommand_from add list done priority edit reopen show dismiss rm next view tui serve bot branch distill accountability scoring remote backfill gen-manpage gen-completions help" -f -a "bot" -d 'Run the Telegram bot (Bot API long-poll)'
